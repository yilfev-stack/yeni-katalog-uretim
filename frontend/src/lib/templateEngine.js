...
function fieldStyle(content, field, defaults = {}) {
  ...
  if (defaults.withBox !== false) s += fieldBoxStyle(c, field);
  return s;
}
  ...
  <div class="page" style="width:794px;height:1123px;${pickPageBackground(d.background_color, `background:${theme.background_color || "#ffffff"};`)}">
    ...
    <div style="display:flex;height:calc(100% - 90px);">
      ...
      <div style="width:58%;padding:28px 24px;display:flex;flex-direction:column;justify-content:center;overflow:hidden;background:${theme.primary_color}08;">
        ...
        ${(isLayerVisible(layerMap,'title') && hasText(d.title)) ? `<h1 style="font-family:'Montserrat',sans-serif;font-size:clamp(18px,3.5vw,28px);font-weight:800;color:${theme.primary_color};line-height:1.2;margin-bottom:12px;${fieldStyle(c,'title',{color:theme.primary_color})}">${esc(d.title)}</h1>` : ''}
        ...
        ${d.bullet_points?.length?`<ul style="list-style:none;padding:0;margin-bottom:16px;${fieldStyle(c,'bullets',{color:'#334155',size:12})}">${hasText(d.label_features) ? `<li style="padding:0 0 6px 0;font-size:9px;font-weight:700;letter-spacing:1px;color:${theme.primary_color};${fieldStyle(c,'label_features',{color:theme.primary_color,size:9})}">${esc(d.label_features)}</li>` : ''}${d.bullet_points.slice(0,7).map(p=>`<li style="padding:4px 0;font-size:12px;color:#334155;display:flex;align-items:flex-start;gap:8px;"><span style="color:${d.bullet_color || '#9ecb2d'};font-size:11px;line-height:1.3;display:inline-block;width:10px;">${d.bullet_style === 'square' || !d.bullet_style ? '&#9632;' : '&#9679;'}</span><span style="flex:1;">${esc(p)}</span></li>`).join('')}</ul>`:''}
    ...
    <div style="padding:60px 50px;height:calc(100% - 90px);display:flex;flex-direction:column;overflow:hidden;">
      ...
      <div style="display:flex;gap:40px;flex:1;overflow:hidden;">
        <div style="flex:1;overflow:hidden;">
          ...
          ${d.bullet_points?.length?`<ul style="list-style:none;padding:0;${fieldStyle(c,'bullets',{color:'#334155',size:12})}">${d.bullet_points.slice(0,8).map(p=>`<li style="padding:8px 0;font-size:12px;color:#334155;border-bottom:1px solid #f1f5f9;">${esc(p)}</li>`).join('')}</ul>`:''}
  ...
  <div class="page" style="width:794px;height:1123px;${pickPageBackground(d.background_color, "background:#fff;")}">
    ...
    <div style="padding:24px;height:calc(100% - 90px);overflow:hidden;">
      ...
      ${d.bullet_points?.length?`<div style="margin-bottom:12px;">${hasText(d.label_features) ? `<div style="background:${theme.primary_color};color:#fff;padding:6px 12px;font-family:'Montserrat',sans-serif;font-size:10px;font-weight:700;letter-spacing:1px;${fieldStyle(c,'label_features',{color:'#ffffff',size:10})}">${esc(d.label_features)}</div>` : ''}
        <table style="width:100%;border-collapse:collapse;${fieldStyle(c,'bullets',{size:11})}">${d.bullet_points.slice(0,10).map((p,i)=>{const pts=p.split(':');return `<tr style="background:${i%2===0?'#f8fafc':'#fff'};"><td style="padding:6px 12px;font-size:11px;color:#334155;border:1px solid #e2e8f0;font-weight:600;width:40%;">${esc(pts[0]||p)}</td><td style="padding:6px 12px;font-size:11px;color:#475569;border:1px solid #e2e8f0;">${esc(pts[1]||'')}</td></tr>`;}).join('')}</table></div>`:''}
      ${d.applications?`<div style="margin-bottom:10px;">${hasText(d.label_applications) ? `<div style="font-size:9px;font-weight:700;color:${theme.primary_color};letter-spacing:1px;margin-bottom:4px;">${esc(d.label_applications)}</div>` : ''}<p style="font-size:11px;color:#475569;line-height:1.5;${fieldStyle(c,'applications')}">${esc(d.applications)}</p></div>`:''}
      ${d.key_benefits?`<div style="background:${theme.primary_color}08;border:1px solid ${theme.primary_color}20;border-radius:6px;padding:12px;">${hasText(d.label_benefits) ? `<div style="font-size:9px;font-weight:700;color:${theme.primary_color};letter-spacing:1px;margin-bottom:4px;">${esc(d.label_benefits)}</div>` : ''}<p style="font-size:11px;color:#334155;line-height:1.5;${fieldStyle(c,'benefits')}">${esc(d.key_benefits)}</p></div>`:''}
    </div>
    ${isLayerVisible(layerMap,'footer') ? footerBar(theme) : ''}
    ${renderShapeLayers(d, layerMap)}
    ${renderOverlayImages(d, layerMap)}
    ...
    <div style="height:calc(100% - 90px);overflow:hidden;">
      ...
      <div style="display:flex;overflow:hidden;">
        <div style="flex:1;padding:24px 32px;overflow:hidden;">
          ...
          ${d.bullet_points?.length?`<div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:18px;${fieldStyle(c,'bullets',{size:11})}">${d.bullet_points.slice(0,8).map(p=>`<div style="background:#f8fafc;padding:10px;border-radius:4px;border-left:3px solid ${theme.primary_color};font-size:11px;color:#334155;">${esc(p)}</div>`).join('')}</div>`:''}
  ...
  <div class="page" style="width:794px;height:1123px;background:linear-gradient(180deg,#020617,#0f172a 50%,#020617);">
    ...
    <div style="padding:40px 36px;height:100%;display:flex;flex-direction:column;overflow:hidden;">
      ...
      <div style="flex:1;display:flex;flex-direction:column;justify-content:center;overflow:hidden;">
        ...
        ${d.bullet_points?.length?`<div style="display:flex;flex-wrap:wrap;gap:6px;margin-bottom:16px;${fieldStyle(c,'bullets',{size:10})}">${d.bullet_points.slice(0,8).map(p=>`<div style="padding:5px 12px;border:1px solid ${neon}30;border-radius:3px;font-size:10px;color:#cbd5e1;background:${neon}08;">${esc(p)}</div>`).join('')}</div>`:''}
