diff --git a/frontend/src/lib/templateEngine.js b/frontend/src/lib/templateEngine.js
index 01948022165c44f26c7cdfb9915f51d444f9c941..b297eb21576cdfc4c610d86b05f06248ab6a46b7 100644
--- a/frontend/src/lib/templateEngine.js
+++ b/frontend/src/lib/templateEngine.js
@@ -1,334 +1,507 @@
...
-  const size = c[`${field}_size`] || defaults.size || 14;
...
-        ${d.image_data ? `<img src="${d.image_data}" style="width:100%;height:100%;object-fit:${d.image_fit||'cover'};" />` : `<div style="width:100%;height:100%;background:linear-gradient(135deg,#1e3a5f,#0f172a);"></div>`}
...
-        <h1 style="font-family:'Montserrat',sans-serif;font-size:clamp(18px,3.5vw,28px);font-weight:800;color:${theme.primary_color};line-height:1.2;margin-bottom:12px;${fieldStyle(c,'title',{color:theme.primary_color})}">${esc(d.title||'Urun Basligi')}</h1>
...
-        ${d.bullet_points?.length?`<ul style="list-style:none;padding:0;margin-bottom:16px;">${d.bullet_points.slice(0,7).map(p=>`<li style="padding:4px 0;font-size:12px;color:#334155;display:flex;align-items:flex-start;gap:6px;${fieldStyle(c,'bullets',{color:'#334155',size:12})}"><span style="color:${theme.primary_color};font-size:14px;line-height:1;">&#9679;</span>${esc(p)}</li>`).join('')}</ul>`:''}
...
-        ${d.image_data?`<img src="${d.image_data}" style="max-width:180px;max-height:160px;object-fit:contain;margin-bottom:20px;border-radius:10px;" />`:''}
...
-          ${d.image_data?`<img src="${d.image_data}" style="width:100%;border-radius:10px;object-fit:${d.image_fit||'cover'};box-shadow:0 16px 48px rgba(0,0,0,0.1);" />`:`<div style="width:100%;aspect-ratio:3/4;background:#f1f5f9;border-radius:10px;display:flex;align-items:center;justify-content:center;"><span style="color:#94a3b8;font-size:12px;">Gorsel</span></div>`}
...
-        ${d.image_data?`<div style="width:160px;flex-shrink:0;"><img src="${d.image_data}" style="width:100%;border-radius:6px;object-fit:${d.image_fit||'contain'};border:1px solid #e2e8f0;" /></div>`:''}
...
+  const layerMap = getLayerMap(d.layers);
   return `<!DOCTYPE html><html><head><style>${BASE_STYLES}</style></head><body>
-  <div class="page" style="width:794px;height:1123px;">
-    ${d.image_data?`<img src="${d.image_data}" style="position:absolute;inset:0;width:100%;height:100%;object-fit:${d.image_fit||'cover'};" />`:`<div style="position:absolute;inset:0;background:linear-gradient(135deg,#1e293b,#0f172a);"></div>`}
+  <div class="page" style="width:794px;height:1123px;${pickPageBackground(d.background_color, `background:${theme.background_color || "#ffffff"};`)}">
...
-        ${d.image_data?`<div style="width:240px;flex-shrink:0;padding:24px 24px 24px 0;"><img src="${d.image_data}" style="width:100%;border-radius:6px;object-fit:${d.image_fit||'cover'};" /></div>`:''}
...
-        ${d.image_data?`<div style="margin-top:14px;"><img src="${d.image_data}" style="max-width:320px;max-height:180px;object-fit:${d.image_fit||'contain'};border-radius:6px;box-shadow:0 0 30px ${neon}20;" /></div>`:''}
...
-      ${d.image_data?`<div style="text-align:center;margin-bottom:16px;"><img src="${d.image_data}" style="max-height:160px;object-fit:${d.image_fit||'contain'};border-radius:10px;" /></div>`:''}
...
-      ${d.image_data?`<img src="${d.image_data}" style="max-width:180px;max-height:180px;object-fit:contain;margin-bottom:24px;border-radius:10px;" />`:''}
