  ...
  <div class="page" style="width:794px;height:1123px;${pickPageBackground(d.background_color, `background:${theme.background_color || "#ffffff"};`)}">
    ...
    <div style="display:flex;height:calc(100% - 90px);">
      ...
      <div style="width:58%;padding:28px 24px;display:flex;flex-direction:column;justify-content:center;overflow:hidden;background:${theme.primary_color}08;">
        ${hasText(d.label_alert) ? `<div style="background:${theme.accent_color||'#f59e0b'};color:#fff;display:inline-block;padding:5px 14px;border-radius:3px;font-family:'Montserrat',sans-serif;font-weight:800;font-size:11px;letter-spacing:2px;margin-bottom:16px;width:fit-content;">${esc(d.label_alert)}</div>` : ''}
        ...
        ${d.applications?`<div style="margin-bottom:12px;">${hasText(d.label_applications) ? `<div style="font-size:9px;font-weight:700;color:${theme.primary_color};letter-spacing:1px;margin-bottom:4px;">${esc(d.label_applications)}</div>` : ''}<p style="font-size:11px;color:#475569;line-height:1.5;${fieldStyle(c,'applications',{color:'#475569',size:11})}">${esc(d.applications)}</p></div>`:''}
        ${d.key_benefits?`<div style="background:${theme.primary_color}10;border-left:3px solid ${theme.primary_color};padding:10px 14px;border-radius:0 6px 6px 0;margin-bottom:14px;">${hasText(d.label_benefits) ? `<div style="font-size:9px;font-weight:700;color:${theme.primary_color};margin-bottom:3px;">${esc(d.label_benefits)}</div>` : ''}<p style="font-size:11px;color:#334155;line-height:1.5;${fieldStyle(c,'benefits',{color:'#334155',size:11})}">${esc(d.key_benefits)}</p></div>`:''}
